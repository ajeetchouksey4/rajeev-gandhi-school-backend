...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        // Seed Facilities Section Photos if missing
        if (galleryItemRepository.countBySection("FACILITIES") == 0) {
            List<GalleryItem> defaultFacilities = List.of(
            ...
            galleryItemRepository.saveAll(defaultFacilities);
        }

        // Seed School Highlights Section Photos if missing
        if (galleryItemRepository.countBySection("HIGHLIGHTS") == 0) {
            List<GalleryItem> defaultHighlights = List.of(
            ...
            galleryItemRepository.saveAll(defaultHighlights);
        }

        // Seed Photo Gallery Section Photos if missing
        if (galleryItemRepository.countBySection("GALLERY") == 0) {
            List<GalleryItem> defaultGallery = List.of(
            ...
            galleryItemRepository.saveAll(defaultGallery);
        }
    }
}
